...
import de.infsec.tpl.modules.appdiff.AppDiff;
...
public class TplCLI {
	...
	public static void main(String[] args) {
		...
		if (LibScoutConfig.opDiff()) {
			if (inputFiles.size() == 2 && isApkFile(inputFiles.get(0)) && isApkFile(inputFiles.get(1))) {
				AppDiff.diffApks(inputFiles.get(0), inputFiles.get(1));
				System.exit(0);
			} else {
				logger.error("You have to provide two apk files");
				usage();
			}
		}
	...
	private static boolean isApkFile(File f) {
		return f.isFile() && f.getName().endsWith(".apk");
	}
	...
	private static void parseCL(String[] args) {
		try {
			...
			if (checkOptionalUse(cmd, CliArgs.ARG_ANDROID_LIB, LibScoutConfig.OpMode.PROFILE, LibScoutConfig.OpMode.MATCH, LibScoutConfig.OpMode.LIB_API_ANALYSIS, LibScoutConfig.OpMode.UPDATABILITY, LibScoutConfig.OpMode.DIFF)) {
			...
			if (checkOptionalUse(cmd, CliArgs.ARG_CONFIG, LibScoutConfig.OpMode.PROFILE, LibScoutConfig.OpMode.MATCH, LibScoutConfig.OpMode.LIB_API_ANALYSIS, LibScoutConfig.OpMode.DB, LibScoutConfig.OpMode.UPDATABILITY, LibScoutConfig.OpMode.DIFF)) {
			...
			if (checkOptionalUse(cmd, CliArgs.ARG_JSON_DIR, LibScoutConfig.OpMode.MATCH, LibScoutConfig.OpMode.LIB_API_ANALYSIS, LibScoutConfig.OpMode.UPDATABILITY, LibScoutConfig.OpMode.DIFF)) {
			...
			if (!(LibScoutConfig.opDB())) {
				...
				if (LibScoutConfig.opLibApiAnalysis()) {
					...
					String[] fileExts = LibScoutConfig.opMatch() || LibScoutConfig.opUpdatability() || LibScoutConfig.opDiff() ? new String[]{"apk"} : new String[]{"jar", "aar"};
	...
	private static void checkRequiredOptions() throws ParseException {
		...
		// android-sdk.jar
		if (Arrays.asList(LibScoutConfig.OpMode.PROFILE, LibScoutConfig.OpMode.MATCH, LibScoutConfig.OpMode.LIB_API_ANALYSIS, LibScoutConfig.OpMode.DIFF).contains(LibScoutConfig.opmode) &&
